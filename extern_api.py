...
# At the end, you should use attach_to_context(value) on the returned value, I'll then relay the value to you
##########################################################
# check for all house smart appliance status
def getHouseStatus()->str:
    return json.dumps(house)

# this API is to take photo from your eye camera, return the photo name
# when I instruct you to look, you can look and see, by taking a photo and then use attach_to_context(photo_name) in the code snippet, I'll then send the photo to you
def camera()->str:
...
# attach_to_context(value)      for reading the value generated by python code for you
# turn_on_photo_stream_mode(on) for turning on continuous photo capture&upload mode on every talk to you
