...
class AutoLangCoquiEngine(BaseEngine):
    ...
    def create_worker_process(self):
        ...
        self.synthesize_process = mp.Process(
            target=AutoLangCoquiEngine._synthesize_worker, args=(
                self.output_queue,
    ...
    def _synthesize_worker(
        ...
        logging.debug(f" - cloning reference {voice}")
        ...
        try:
            ...
            gpt_cond_latent, speaker_embedding = get_conditioning_latents(voice, tts)
    ...
    def set_cloning_reference(self, voice: str):
        """
        ...
        if status == 'success':
            ...
            logging.error(f'Error updating reference WAV: {voice}')
